...
class MaskedAtariEnv(gym.Env):
    ...
    def setup(self, masker_definitions=FROSTBITE_MASKER_DEFINITIONS, zero_mask_indices=None):
        ...
        self.all_colors = np.stack([np.ones(self.observation_space.shape, dtype=np.uint8) * c for c in all_colors])
        self.category_lengths = [len(masker.filter_colors) for masker in self.masker_definitions]
        self.zero_mask = np.ones(len(self.masker_definitions), *self.observation_space.shape, dtype=np.uint8)
    ...
    def step(self, action):
        ...
        category_masks = np.zeros(len(self.masker_definitions), *self.observation_space.shape, type=np.uint8)
